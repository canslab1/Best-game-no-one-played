...
to setup-social-network
   ...
   ask-concurrent links [ set color gray - 4 ]
   display
...
to communicate [ A1 A2 B C ]
   if (A1  = true and A2 != true and abs(C - B) < max-opinion-distance) [ ifelse (B > C) [ change-opinion-2 B C ] [ change-opinion-1 B C ] ] ; 隔壁已採用, 本身未採用, 若隔壁態度比我正面, 單向說服, 若隔壁態度比我負面, 雙向溝通
   if (A1 != true and A2  = true and abs(C - B) < max-opinion-distance) [ ifelse (B > C) [ change-opinion-1 B C ] [ change-opinion-3 B C ] ] ; 自己已採用, 隔壁未採用, 若隔壁態度比我正面, 雙向溝通, 若隔壁態度比我負面, 單向說服
   if (A1  = true and A2  = true and abs(C - B) < max-opinion-distance) [ ifelse (B > C) [ change-opinion-2 B C ] [ change-opinion-3 B C ] ] ; 隔壁和自己皆採用, 態度變得更正面                                      
   if (A1 != true and A2 != true and abs(C - B) < max-opinion-distance)                  [ change-opinion-1 B C ]                            ; 自己和隔壁皆未採用, 雙向溝通, 拉近距離
end
...
to update-plot
   set-current-plot "採納動態圖"
   set-plot-y-range 0 count turtles
   set-plot-x-range 1 max-time
   set-current-plot-pen "未採納者"
   plot (count turtles with [ not act ])
   set-current-plot-pen "採納者"
   plot (count turtles with [ act ])
   
   set-current-plot "新採納者動態圖"
   set-plot-x-range 1 max-time
   set-current-plot-pen "新採納者"
...
true
PENS
"採納者" 1.0 0 -2674135 true
"未採納者" 1.0 0 -10899396 true
